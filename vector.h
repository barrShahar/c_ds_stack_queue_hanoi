 ...
 * @retval VECTOR_NULL_PTR if _vector is null, if *_vector is null
 * @retval VECTOR_UNINITIALIZED_ERROR if _vector->m_voidArray is null
 * @retval VECTOR_OVERFLOW_ERROR if _vector->m_blockSize is 0
 * @retval VECTOR_ALLOCATION_ERROR if realloc fails 
 */
 ...
 * @return success or error code 
 * @retval VECTOR_SUCCESS on success 
 * @retval VECTOR_NULL_PTR if _vector, if p_Value is null
 * @retval VECTOR_UNINITIALIZED_ERROR if the vector's members are uninitialized 
 * @retval VECTOR_EMPTY_ERROR if _vector is empty
 ...
 * @retval VECTOR_NULL_PTR if _vector, if p_Value is null
 * @retval VECTOR_UNINITIALIZED_ERROR if the vector's members are uninitialized 
 * @retval VECTOR_INDEX_OUT_OF_BOUNDS_ERROR if the index exceeds the vector bounds
 */
 ...
 * @retval VECTOR_NULL_PTR if _vector
 * @retval VECTOR_UNINITIALIZED_ERROR if the vector's members are uninitialized 
 * @retval VECTOR_INDEX_OUT_OF_BOUNDS_ERROR if the index exceeds the vector bounds
 * 
 */
